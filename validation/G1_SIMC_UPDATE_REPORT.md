...
- 状态：已完成
- 首次完成时间：2026-08-21 09:30:28 +0800
- 上游续扫完成时间：2026-08-21 10:24:02 +0800
- 基线目标：G0 `20260821-g0`
...
差异扫描器已对官方 `midnight` 的 `70c46902f3a1218a5d03c46607d07ec443ffe356` / 12.1.0.69404 执行真实扫描。该目标是首次报告所用 `b458aea6898f3b169310ed243e9934bfa37044bd` 之后的最新续扫目标；两者之间有 3 个提交，内容为 Build 69404 数据刷新、移除 Venomcursed Ascendance 临时兼容 hack，以及重新生成数据。7 个目标文件中 5 个有差异，`sc_druid.cpp` 与 Feral APL 完全未变。扫描出的 17 条机制候选已通过绑定 commit 和精确计数的机读 review 逐段复核，硬门禁结果为：`unreviewedMechanismCount=0`、`silentUnsupportedDropCount=0`、`aplReferencesMissingActionCount=0`、`unknownChangeLineCount=0`。
...
项目根目录已在 G1 首次完成后建立 Git：当前发布基线为 `main` / `36155167239dfd2ea3d2da856f6f7c6902fdfb48`，并与 GitHub `origin/main` 一致。原 G0 和本报告首次版本中“项目无 Git”的描述仅代表当时快照；`vendor/simc` 无 Git、产品 SimC 快照为 mixed 的事实没有变化。

`profileCacheNamespace` 本轮只作为版本元数据生成，现有 `app.js` 的 localStorage key 未迁移；缓存迁移必须在后续目标中单独设计和回归，不能在 G1 顺带修改 UI 状态逻辑。

## 3. 上游扫描结果

目标：官方 SimulationCraft `midnight`，commit `70c46902f3a1218a5d03c46607d07ec443ffe356`，WoW `12.1.0.69404`，上游数据说明为 hotfix `2026-08-21 / 69404`。

| 分类 | 变更行数 |
| --- | ---: |
| `version_only` | 24 |
| `talent` | 0 |
| `action_number_or_resource` | 12234 |
| `timing_or_cooldown` | 1 |
| `aura_or_dot` | 17 |
| `apl` | 0 |
| `tier_set` | 0 |
| `gear_or_trinket` | 2722 |
| `new_generic_mechanism` | 10 |
| `custom_mechanism_candidate` | 7 |
| `unknown` | 0 |

总计 15015 个增删行。`trait_data.inc` 和 `item_set_bonus.inc` 仅构建号头部变化；`sc_spell_data.inc` 与 `item_effect.inc` 有全局生成数据及索引位移，不能仅按行数推断 Feral 语义变化。Feral APL 的 26 个动作引用保持不变，没有新增或缺失动作。

`unique_gear_midnight.cpp` 的语义差异分三组：SimC 统计归属 bookkeeping、既有装备 Buff 刷新修正、以及 `venomcursed_ascendance` 随机属性回调。续扫目标已把装备 `237622` 从临时 driver `1317036` 正式切换到 driver `1317582` / buff `1317581`，并删除临时兼容分支。`237622`、`1317581`、`1317582` 均不在当前 MID1 Profile、装备 oracle 或训练器目录中；G1 不实现它。未来若采用包含这些标识的构筑，必须先产生结构化 unsupported item-effect，直到有通用机制处理器，禁止静默忽略。

机读 review 会校验目标 commit 及两类候选的精确行数（10 / 7）。上游或分类器发生漂移时扫描会直接失败，旧 review 不能自动放行新差异。
...
| 目标扫描 | 7 文件；5 变、2 不变；15015 个增删行 |
| 机制候选 review | 17 已审；`unreviewedMechanismCount=0` |
| 静默未支持丢弃 | `silentUnsupportedDropCount=0` |
| APL 缺失动作 | 26→26；新增 0；缺失 0 |
| 未分类变化 | 0 |
| `npm test` | 48/48 通过，0 失败；新增报告/review/硬门禁绑定测试 |
...
续扫完成后的浏览器烟测一次通过；桌面 1828×1028、H5 390×844、构筑切换、1/3/5 目标、SimC 导入、非法 Profile 保留、XSS 纯文本和原生/回退弹窗均通过，临时页和静态服务均已关闭。
...
git -C /private/tmp/wow-feral-simc-git-70c46902f3a fetch --depth 1 --filter=blob:none origin 70c46902f3a1218a5d03c46607d07ec443ffe356
git -C /private/tmp/wow-feral-simc-git-70c46902f3a checkout --detach FETCH_HEAD
node scripts/scan-simc-update.mjs \
  --target-root /private/tmp/wow-feral-simc-git-70c46902f3a \
  --target-commit 70c46902f3a1218a5d03c46607d07ec443ffe356 \
...
- 项目现在有 Git 元数据，当前续扫改动以 `36155167239dfd2ea3d2da856f6f7c6902fdfb48` 为修改前 Git 回滚点；`vendor/simc` 仍没有 Git 元数据，SimC 输入恢复仍需 G0 快照。
...
续扫修改前 Git 回滚点为 `36155167239dfd2ea3d2da856f6f7c6902fdfb48`；后续若提交本轮变更，应通过可审计的反向提交或按文件恢复处理，不执行破坏性 reset。SimC 输入的灾备回滚点仍为 `validation/baselines/20260821-g0/project-source.tar.gz`（SHA-256 `354f741c...4c49`）和 `simc-scan-inputs.tar.gz`（SHA-256 `054e0828...0bb5`）。恢复时先解压到临时目录、核对 `project-source.sha256`，再按报告清单选择性恢复；禁止直接覆盖工作区或删除用户文件。因为 G1 没有部署，不需要远端回滚。
