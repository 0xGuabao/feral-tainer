...
const siteFiles = ["index.html", "wow-hud.css", "reference-layout.css", "app.js", "feral-catalog.js", "feral-engine.js", "trainer-controller.js"];
const siteDirectories = ["apl", "assets", "core", "data", "runtime"];

function copySite(destination) {
  const demoDestination = join(destination, "demo");
  mkdirSync(demoDestination, { recursive: true });
  for (const name of siteFiles) cpSync(join(repositoryRoot, "demo", name), join(demoDestination, name));
  for (const name of siteDirectories) cpSync(join(repositoryRoot, "demo", name), join(demoDestination, name), { recursive: true });
}
